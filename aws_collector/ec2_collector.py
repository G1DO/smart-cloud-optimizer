"""
ec2_collector.py — EC2 inventory collector.

Collects EC2 instances, EBS volumes, and region information
across all enabled AWS regions and saves to consolidated CSV.
...
class EC2Collector:
    ...
    def list_regions(self) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"[Inventory] Found {len(regions_data)} regions")
        except Exception as e:
    ...
    def save_inventory(self):
        ...
        if instances:
            ...
            logger.info(f"[Inventory] ✓ Saved {len(instances)} instances to {instances_file.name}")
        else:
            ...
            with open(instances_file, 'w', newline='') as f:
                writer = csv.DictWriter(f, fieldnames=fieldnames)
                writer.writeheader()
            logger.info(f"[Inventory] ✓ Created empty {instances_file.name}")
        ...
        if volumes:
            ...
            logger.info(f"[Inventory] ✓ Saved {len(volumes)} volumes to {volumes_file.name}")
        else:
            # Create empty CSV with standard fields
            fieldnames = [
            ...
            with open(volumes_file, 'w', newline='') as f:
                writer = csv.DictWriter(f, fieldnames=fieldnames)
                writer.writeheader()
            logger.info(f"[Inventory] ✓ Created empty {volumes_file.name}")
        ...
        if regions:
            ...
            logger.info(f"[Inventory] ✓ Saved {len(regions)} regions to {regions_file.name}")
        else:
            fieldnames = ['region_name', 'endpoint']
            with open(regions_file, 'w', newline='') as f:
                writer = csv.DictWriter(f, fieldnames=fieldnames)
                writer.writeheader()
            logger.info(f"[Inventory] ✓ Created empty {regions_file.name}")
