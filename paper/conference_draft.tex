    ...
    % Authorship switch. Named final mode is the default used by the normal
    % build; change to \anonymoustrue only for an anonymous-review build.
    \newif\ifanonymous
    \anonymousfalse
...
\input{table_catalog_comparison.tex}

\input{table_baselines.tex}

\input{table_service_decomposition.tex}

\input{table_ablations.tex}

\input{table_runtime.tex}

\input{table_commercial_like.tex}
...
\input{table_external_baselines_compact.tex}
...
\centering
\renewcommand{\arraystretch}{1.08}
\input{external_mape_tabular.tex}
\end{table}
