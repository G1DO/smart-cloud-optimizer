...
Outputs (written under paper/):
  ...
  cv_results.csv                    per-(model,horizon) CV MAPE table (mean + per-fold sd)
...
import paperlib  # noqa: E402  (paper-local evaluation helpers, same dir)
...
MASE_M = 7                # seasonal period of the MASE denominator (weekly naive)
STAT_H = 30               # headline horizon for MASE/coverage/DM/Wilcoxon statistics
RNG_SEED = 0              # Prophet simulates its intervals via the global NumPy RNG
...
def run_cv(df, with_sarimax=False):
    ...
    if with_sarimax:
        factories["SARIMAX"] = lambda: SARIMAXForecaster()

    table = {}        # model -> {horizon -> mape_mean}
    sd = {}           # model -> {horizon -> per-fold mape sd (ddof=0)}
    folds = {}        # model -> {horizon -> n_folds}
    detail = {}       # model -> {horizon -> full paperlib.walk_forward result}
    for h in HORIZONS:
        for model, factory in factories.items():
            # Point forecasts are deterministic; Prophet's Monte-Carlo interval
            # sampling is not unless the global RNG is seeded per evaluation.
            np.random.seed(RNG_SEED)
            res = paperlib.walk_forward(factory, df, date_col="date", value_col="total_cost",
                                        initial=CV_INITIAL, horizon=h, step=CV_STEP, m=MASE_M)
            mapes = [f["mape"] for f in res["folds"]]
            table.setdefault(model, {})[h] = float(np.nanmean(mapes))
            sd.setdefault(model, {})[h] = float(np.nanstd(mapes))  # ddof=0, as in compare_models
            folds.setdefault(model, {})[h] = len(res["folds"])
            detail.setdefault(model, {})[h] = res
        ranked = sorted(factories, key=lambda m: table[m][h])
        print(f"  horizon={h:>3}d  " + "  ".join(f"{m}={table[m][h]:.1f}%" for m in ranked))
    # Order models by MAPE at the first horizon -- the ordering compare_models
    # used to establish, inherited by numbers.tex / cv_results.csv / the figure.
    order = sorted(table, key=lambda m: table[m][HORIZONS[0]])
    return ({m: table[m] for m in order}, {m: sd[m] for m in order},
            {m: folds[m] for m in order}, {m: detail[m] for m in order})


def run_stats(detail):
    """Statistics at the headline STAT_H-day horizon: per-model mean per-fold
    MASE and pooled per-point 95%-interval coverage, plus paired significance
    tests for the two front-runners (ETS vs SeasonalNaive): Diebold-Mariano
    with HLN correction on pooled aligned errors, and Wilcoxon signed-rank on
    per-fold MAPEs. Every fold spans exactly STAT_H points (walk_forward only
    keeps complete folds), so the mean of per-fold coverages equals the pooled
    per-point coverage.
    """
    mase, coverage = {}, {}
    for model, hmap in detail.items():
        fl = hmap[STAT_H]["folds"]
        mase[model] = float(np.nanmean([f["mase"] for f in fl]))
        coverage[model] = float(np.nanmean([f["coverage"] for f in fl]))
    ets, sn = detail["ETS"][STAT_H], detail["SeasonalNaive"][STAT_H]
    dm_stat, dm_p, dm_dbar = paperlib.dm_test(ets["y_true"] - ets["y_pred"],
                                              sn["y_true"] - sn["y_pred"], h=STAT_H, power=1)
    w_stat, w_p = paperlib.wilcoxon_folds([f["mape"] for f in ets["folds"]],
                                          [f["mape"] for f in sn["folds"]])
    return {
        "horizon": STAT_H,
        "n_folds": len(ets["folds"]),
        "mase": mase,
        "coverage": coverage,
        "dm_ets_vs_snaive": {"stat": dm_stat, "p": dm_p, "mean_loss_diff": dm_dbar,
                             "loss": "absolute error", "h": STAT_H},
        "wilcoxon_mape_ets_vs_snaive": {"stat": w_stat, "p": w_p},
    }
...
def fig_architecture():
    ...
    box(53, 15.5, 20, 9, "optimizer: MILP right-size\n+ 8 rules  (fixed-fractile\n+ always-on RI gating)", C["orange"], tc="black")
...
def fig_daily_surges(df, surges, n):
    fig, ax = plt.subplots(figsize=(DBLW, 1.85))
...
def fig_forecast_holdout(train, test, fc, mape, cov, model_name):
    fig, ax = plt.subplots(figsize=(COLW, 2.1))
    tail = train.tail(60)
...
def fig_mape_horizon(table, sd):
    ...
    for model, hmap in table.items():
        ...
        xs = sorted(hmap)
        mean = [hmap[x] for x in xs]
        band = [sd[model][x] for x in xs]
        ax.fill_between(xs, [m - s for m, s in zip(mean, band)],
                        [m + s for m, s in zip(mean, band)], color=col, alpha=0.15, lw=0)
        ax.plot(xs, mean, color=col, marker=mk, ls=ls, ms=3.5, label=model)
...
def fig_savings_breakdown(n):
    ...
    y = range(len(items))
    fig, ax = plt.subplots(figsize=(COLW, 2.15))
...
def sig2(x):
    ...
    if not np.isfinite(x) or x == 0:
        return "0"
    d = 1 - int(np.floor(np.log10(abs(x))))
    return f"{x:.{max(d, 0)}f}"


def fmt_p(p):
    """p-value to 3 decimals, floored at the reporting threshold."""
    return "<0.001" if p < 0.001 else f"{p:.3f}"


def write_numbers_tex(n, table, sd, stats, folds, holdout_mape, holdout_cov, holdout_model, with_sarimax):
    L = []
    L.append("% Auto-generated by paper/make_figures.py -- do not edit by hand.\n")
    L.append("% Legacy stored-recommendation macros in this file are not the active\n")
    L.append("% Study I evidence for conference_draft.tex. Use numbers_evidence.tex\n")
    L.append("% for repaired snapshot-repriced synthetic-account results.\n")
    ...
    # ---- statistical rigor at the headline 30-day horizon ----
    for model, hmap in table.items():
        if 30 not in hmap:
            continue
        sfx = mname.get(model, model)
        L.append(latexcmd(f"mapeSd{sfx}", sig2(sd[model][30])))
        L.append(latexcmd(f"mase{sfx}", f"{stats['mase'][model]:.2f}"))
        L.append(latexcmd(f"cov{sfx}", f"{stats['coverage'][model]:.0f}"))
    L.append(latexcmd("dmStatEtsSnaive", f"{stats['dm_ets_vs_snaive']['stat']:.2f}"))
    L.append(latexcmd("dmPEtsSnaive", fmt_p(stats["dm_ets_vs_snaive"]["p"])))
    L.append(latexcmd("wilcoxonPEtsSnaive", fmt_p(stats["wilcoxon_mape_ets_vs_snaive"]["p"])))
    L.append(latexcmd("nFoldsThirty", f"{stats['n_folds']}"))
    L.append(latexcmd("minFolds", f"{min(nf for hmap in folds.values() for nf in hmap.values())}"))

    (PAPER / "numbers.tex").write_text("".join(L))


def write_mape_table(table, sd):
    ...
    of the alignment. Cells are mean $\\pm$ per-fold sd; the best (minimum) mean
    in each horizon column is bold-faced. The mean-plus-sd cells outgrow the IEEE
    single column, so the tabular is wrapped in \\resizebox{\\columnwidth}
    (graphicx, loaded by main.tex) with tightened \\tabcolsep -- both scoped to
    the table float.
    """
    ...
    out = [r"\setlength{\tabcolsep}{3pt}%", r"\resizebox{\columnwidth}{!}{%",
           r"\begin{tabular}{@{}l " + "r " * len(DISPLAY_H) + r"@{}}", r"\toprule"]
...
def main():
    ...
    table, sd, folds, detail = run_cv(df, with_sarimax=args.with_sarimax)
    stats = run_stats(detail)
    print(f"      h={STAT_H}d stats ({stats['n_folds']} folds): "
          f"DM(ETS vs SeasonalNaive) stat={stats['dm_ets_vs_snaive']['stat']:.2f} "
          f"p={fmt_p(stats['dm_ets_vs_snaive']['p'])}; "
          f"Wilcoxon p={fmt_p(stats['wilcoxon_mape_ets_vs_snaive']['p'])}")
    ...
    fig_mape_horizon(table, sd)
    fig_savings_breakdown(n)
    fig_service_share(n)
    write_numbers_tex(n, table, sd, stats, folds, holdout_mape, holdout_cov, holdout_model, args.with_sarimax)
    write_mape_table(table, sd)

    # cv csv (mean columns, then per-fold sd columns)
    cv_rows = ["model," + ",".join(f"h{h}" for h in HORIZONS)
               + "," + ",".join(f"h{h}_sd" for h in HORIZONS)]
    for model, hmap in table.items():
        cv_rows.append(model + "," + ",".join(f"{hmap.get(h, float('nan')):.2f}" for h in HORIZONS)
                       + "," + ",".join(f"{sd[model].get(h, float('nan')):.2f}" for h in HORIZONS))
    (PAPER / "cv_results.csv").write_text("\n".join(cv_rows) + "\n")

    per_fold = {model: {h: [round(f["mape"], 4) for f in detail[model][h]["folds"]]
                        for h in detail[model]} for model in detail}
    out = {"numbers": n, "cv_mape": table, "cv_mape_sd": sd, "cv_folds": folds,
           "cv_mape_per_fold": per_fold,
           "stats": {"horizon": stats["horizon"], "n_folds": stats["n_folds"],
                     "mase": {m: round(v, 3) for m, v in stats["mase"].items()},
                     "coverage": {m: round(v, 2) for m, v in stats["coverage"].items()},
                     "dm_ets_vs_snaive": {k: (round(v, 6) if isinstance(v, float) else v)
                                          for k, v in stats["dm_ets_vs_snaive"].items()},
                     "wilcoxon_mape_ets_vs_snaive": {k: round(v, 6)
                                                     for k, v in stats["wilcoxon_mape_ets_vs_snaive"].items()}},
           ...
           "cv_config": {"initial": CV_INITIAL, "step": CV_STEP, "horizons": HORIZONS,
                         "mase_m": MASE_M, "with_sarimax": args.with_sarimax}}
