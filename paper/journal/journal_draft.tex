...
% Source conference manuscript: ../conference_draft.tex
% This file intentionally uses a neutral article class rather than a
% journal-specific template. It reads generated evidence artifacts from ../.
...
\input{../numbers.tex}
\input{../numbers_external.tex}
\input{../numbers_azure.tex}
\input{../numbers_policy.tex}
\input{../numbers_policy_ext.tex}
\input{../numbers_evidence.tex}
...
% Authorship switch:
% - Keep \anonymousfalse for named submission.
% - Change to \anonymoustrue for anonymous review.
% Identifying author metadata and acknowledgments are conditional on this
% switch. Confirm the target journal's review model before submission.
\newif\ifanonymous
\anonymousfalse

\title{The Price of Conservatism: Empirical Evidence from Cloud Right-Sizing and Reservation Heuristics}
\hypersetup{
  pdftitle={The Price of Conservatism: Empirical Evidence from Cloud Right-Sizing and Reservation Heuristics}
}
...
\else
\hypersetup{pdfauthor={Ibrahim Mohamed; Mariam Emad; Hazem Ibrahim; Ahmed Sameh; Mahmoud Ahmed; John Ehab}}
\author{\small Ibrahim Mohamed, Mariam Emad, Hazem Ibrahim, Ahmed Sameh, Mahmoud Ahmed, John Ehab\\[3pt]
...
reservation; reserved instances; newsvendor; FinOps; utilization percentiles;
...
checksum is unavailable until \texttt{vmtable.csv.gz} is restored.}
\label{tab:datasets}
\centering
\footnotesize
...
Azure Public Dataset V2 & \azVms{} filtered VMs from \azTotalRows{} rows, 2019 trace & recorded boundary case & \path{paper/results_azure.json}; raw checksum pending \\
\bottomrule
\end{tabularx}
\end{table*}
...
\input{../table_service_decomposition.tex}
...
\texttt{paper/trace\_provenance.md}; Azure raw parsing runtime is pending
restoration of \texttt{vmtable.csv.gz}. These diagnostics are not a
...
are not a hardware-normalized scalability benchmark, and a full
process-memory and dependency-version inventory remains pending.
...
\setlength{\tabcolsep}{3pt}
\input{../table_external_baselines.tex}
\end{table*}
...
The principal rerun path is documented, but dependency pinning and clean-room
verification remain pending. Azure is not reproducible in this workspace
until \texttt{vmtable.csv.gz} is restored and rerun. Before public artifact
release, the authors must add the repository URL and archive identifier,
license, supported operating systems, Python and solver versions, dependency
lock information, LaTeX engine version, execution hardware, and final
...
% TODO before submission: finalize the data-availability statement, public
% repository URL, and archive DOI.
\textbf{Data availability.} Synthetic data and generated outputs are in the
repository. GWA restoration commands and checksums are recorded in
\path{paper/trace_provenance.md}.

The Azure raw \texttt{vmtable.csv.gz} remains unrestored.

% TODO before submission: finalize the code-availability statement and archive
% DOI.
\textbf{Code availability.} Manuscript scripts are identified above by
artifact-relative filenames in the paper artifact directory.

% TODO before submission: confirm the competing-interests declaration.
% TODO before submission: confirm the funding declaration.
% TODO before submission: confirm the ethics and consent-to-participate
% declaration.
% TODO before submission: confirm the consent-for-publication declaration.
% TODO before submission: complete any disclosure required by the selected
% journal's generative-AI policy.
