...
const searchRoutes = [
  {
    label: "Dashboard",
    description: "Overview, account monitoring, KPIs, and executive snapshot",
    path: "/dashboard/home",
    terms: ["home", "dashboard", "overview", "summary", "account", "monitoring", "main", "kpi", "snapshot"],
  },
  {
    label: "Costs",
    description: "Daily spend, service breakdown, billing analysis, and cost tables",
    path: "/dashboard/costs",
    terms: ["cost", "costs", "spend", "spending", "billing", "analysis", "service", "services", "ec2", "rds", "s3", "daily", "breakdown"],
  },
  {
    label: "Forecasts",
    description: "Prediction models, future spend, risk trend, and forecast table",
    path: "/dashboard/forecasts",
    terms: ["forecast", "forecasts", "prediction", "predict", "risk", "trend", "next month", "model", "prophet", "sarimax"],
  },
  {
    label: "Recommendations",
    description: "Savings actions, priorities, optimization queue, and recommendation cards",
    path: "/dashboard/recommendations",
    terms: ["recommendation", "recommendations", "saving", "savings", "optimize", "optimization", "rightsizing", "idle", "priority", "medium"],
  },
  {
    label: "Guided Questions",
    description: "AI onboarding questions and prompt builder",
    path: "/dashboard/guided-questions",
    terms: ["guided", "questions", "question", "prompt", "builder", "ai", "onboarding", "setup"],
  },
  {
    label: "Account Settings",
    description: "Profile, workspace, AWS connections, and runtime settings",
    path: "/dashboard/account-settings",
    terms: ["settings", "profile", "connection", "connections", "aws connection", "account settings", "aws account", "workspace"],
  },
] satisfies SearchTarget[];

const searchCardTargets: SearchTarget[] = [
  {
    label: "Total Cloud Cost",
    description: "Dashboard cost KPI card",
    path: "/dashboard/home",
    terms: ["total cloud cost", "cloud cost", "cost card", "monthly cost", "kpi"],
  },
  {
    label: "Potential Savings",
    description: "Dashboard savings KPI card",
    path: "/dashboard/home",
    terms: ["potential savings", "savings card", "monthly savings", "kpi"],
  },
  {
    label: "Anomalies Detected",
    description: "Dashboard anomaly KPI card",
    path: "/dashboard/home",
    terms: ["anomalies", "anomaly", "detected", "risk", "kpi"],
  },
  {
    label: "Total Recommendations",
    description: "Recommendations summary card",
    path: "/dashboard/recommendations",
    terms: ["total recommendations", "recommendations card", "optimization opportunities"],
  },
  {
    label: "Medium Priority",
    description: "Medium-priority recommendations card",
    path: "/dashboard/recommendations",
    terms: ["medium priority", "priority card", "medium recommendations"],
  },
  {
    label: "Forecast Model",
    description: "Forecast model selection and prediction chart",
    path: "/dashboard/forecasts",
    terms: ["forecast model", "model card", "prediction chart", "prophet", "sarimax"],
  },
  {
    label: "AWS Connections",
    description: "Connect and manage AWS accounts",
    path: "/dashboard/account-settings?tab=connections",
    terms: ["aws connections", "connect aws", "manage connections", "account settings"],
  },
];

const searchTargets = [...searchRoutes, ...searchCardTargets];
...
export default function DashboardBar({
  ...
  const navigateToSearchTarget = (target: SearchTarget) => {
    setLanguageOpen(false);
    setProfileOpen(false);
    setSearchOpen(false);
    setSearchQuery("");
  ...
  const handleSearchSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const match =
      ...
      searchTargets.find((target) =>
        target.terms.some((term) => query.includes(term) || term.includes(query))
      );
    ...
    setSearchOpen(false);
    setSearchQuery("");
    router.push(match?.path ?? `/dashboard/home?search=${encodeURIComponent(query)}`);
  };
